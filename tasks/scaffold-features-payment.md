# scaffold-features-payment Implement payment feature scaffold

## Metadata
- **Task ID**: scaffold-features-payment
- **Owner**: AGENT
...
## Context
`packages/features/src/payment/index.ts` is scaffolded but not implemented, leaving an incomplete public surface in `packages/features/src/payment`. This creates capability gaps and blocks roadmap-aligned adoption.
...
## Related Files
- `packages/features/src/payment/` — implement package internals (modify/create)
- `packages/features/src/payment/index.ts` — replace scaffold/stub entrypoint (modify)
- `tasks/scaffold-features-payment.md` — implementation contract (this file)
...
```ts
// packages/features/src/payment/index.ts
