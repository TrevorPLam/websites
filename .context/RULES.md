...
**These apply to every task. A task is not complete until all are satisfied.**

### 1. THEGOAL alignment

- Analyze [THEGOAL.md](../THEGOAL.md) before starting
- Understand repository direction (136 tasks, waves, target architecture)
- Ensure work aligns with CaCA, layer model, and planned structure

### 2. Research (per task)

- Include up-to-date (02/2026) information: basics, fundamentals, best practices, highest standards
- Conduct research if current info does not exist
- Cite sources when applicable

### 3. Examples and guidance

- Include code snippets or similar examples where applicable
- Derive examples from (a) up-to-date research, (b) current implementation
- Guide execution with concrete patterns

### 4. Refactor tasks

- If research contradicts existing structures/implementations but suggests better practices, create additional refactor tasks
- Do not silently diverge; capture improvements as follow-up work

### 5. Quality assurance (three passes)

- **Pass 1:** In isolation — validate the changed files alone
- **Pass 2:** With related files — verify integration, imports, dependent modules
- **Pass 3:** Full codebase context — ensure no regressions, run full `pnpm lint`, `pnpm type-check`, `pnpm test`, `pnpm build`

### 6. Testing and validation (required)

- Create or update tests per task
- Implement and run validation (e.g. `pnpm validate-exports`, `pnpm validate-client`)
- **Task is incomplete without this step**

### 7. Documentation (required)

- Meta headers in source files (per file header standard)
- Inline code commentary where non-obvious
- Repository documentation (README, docs/, task files) updated
- **Task is incomplete without this step**

### 8. Additional tasks

- After all above are complete, determine whether any new tasks need to be created
- Log findings in task tracking (TASKS.md, tasks/)

### 9. Lessons learned

- Analyze the work and any lessons learned
- Attach insights to related tasks and documentation
- Update docs/tasks with retrospective notes where applicable

### 10. Commit and push

- After each task is complete, commit with a descriptive message
- Push to GitHub
...
_These rules ensure consistency, quality, and maintainability across the marketing-websites platform._
