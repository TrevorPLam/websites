...
export const LeadSchema = z.object({
  ...
  tenantId: z.string().uuid('Invalid tenant ID format').describe('Tenant identifier for multi-tenant isolation'),

  // Contact information
  email: z.string().email('Invalid email format').max(254, 'Email too long').describe('Primary email address'),
  name: z.string().min(1, 'Name is required').max(100, 'Name too long').describe('Full name of the lead'),
  phone: PhoneNumberSchema.describe('Phone number in E.164 format'),
  company: z.string().max(100, 'Company name too long').optional().describe('Company or organization name'),
  ...
  content: z.string().max(100, 'Content too long').optional().describe('Marketing content identifier'),
  ...
  }).optional().describe('GDPR/CCPA consent data')
})
...
export const LeadSearchSchema = z.object({
  ...
  search: z.string().max(100, 'Search term too long').optional().describe('Search in name, email, company'),
  limit: z.number().int().min(1).max(100).default(20).describe('Number of results to return'),
  offset: z.number().int().min(0).default(0).describe('Number of results to skip'),
  sortBy: z.enum(['createdAt', 'updatedAt', 'name', 'email', 'score']).default('createdAt').describe('Sort field'),
...
export const validateLead = (data: unknown): Lead => {
  try {
    ...
    if (error instanceof z.ZodError) {
      const formattedError = new Error(`Lead validation failed: ${error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join(', ')}`)
...
export const validateCreateLead = (data: unknown): CreateLeadData => {
  try {
    ...
    if (error instanceof z.ZodError) {
      const formattedError = new Error(`Create lead validation failed: ${error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join(', ')}`)
...
export const validateUpdateLead = (data: unknown): UpdateLeadData => {
  try {
    ...
    if (error instanceof z.ZodError) {
      const formattedError = new Error(`Update lead validation failed: ${error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join(', ')}`)
...
export const validateLeadSearch = (data: unknown): LeadSearchParams => {
  try {
    ...
    if (error instanceof z.ZodError) {
      const formattedError = new Error(`Lead search validation failed: ${error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join(', ')}`)
