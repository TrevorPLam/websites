...
export const SkillManifestSchema = z.object({
  ...
  compatibility: z.object({
    mcpVersion: z.string()
      .regex(/^\d+\.\d+\.\d+$/, 'MCP version must follow semantic versioning'),
    
    nodeVersion: z.string()
      .regex(/^\d+\.\d+$/, 'Node version must be in format x.y'),
  ...
  // Dependencies
  dependencies: z.record(z.string().regex(/^\d+\.\d+\.\d+$/))
    .optional()
    .default({}),
  ...
  security: z.object({
    ...
    sandboxLevel: z.enum(['none', 'basic', 'strict', 'isolated'])
      .default('basic'),
  ...
  multiTenant: z.object({
    ...
    isolationLevel: z.enum(['none', 'logical', 'strict', 'isolated'])
      .default('none'),
