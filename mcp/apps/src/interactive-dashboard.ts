...
/**
 * Interactive dashboard MCP application for real-time data visualization.
 *
 * Provides tools for generating, updating, and exporting dashboard data
 * with interactive UI components for data exploration and analysis.
 */
export class InteractiveDashboardMCPApp {
  ...
  private setupTools() {
    ...
    this.server.tool(
      ...
      {
        dataType: z
          .enum(['sales', 'analytics', 'system', 'performance'])
  ...
  private setupUIHandlers() {
    ...
    this.app.ui.handlers.push({
      event: 'data-selected',
      handler: async (data) => {
        const selectedItems = this.data.filter((item: any) => data.selectedIds.includes(item.id));
