#!/usr/bin/env tsx
/**
 * @file scripts/migrate-legacy-guides.ts
 * @summary Systematically migrates high-value content from legacy docs/guides/ to unified docs/ structure.
 * @description Analyzes legacy documentation, prioritizes content for migration, and performs structured migration with proper frontmatter validation.
 * @security Validates all file paths and content to prevent injection attacks during migration.
 * @adr docs/architecture/decisions/ADR-006-legacy-migration.md
...
class LegacyGuidesMigrator {
  ...
  private async executeMigration(report: MigrationReport): Promise<void> {
    ...
    const allContent = [
      ...report.migrationPlan.highPriority,
      ...report.migrationPlan.mediumPriority,
      ...report.migrationPlan.lowPriority,
    ];
    ...
    for (let i = 0; i < allContent.length; i += batchSize) {
      ...
      for (const item of batch) {
        try {
          await this.migrateFile(item);
          report.migratedFiles++;
          item.migrationStatus = 'migrated';
        } catch (error) {
          console.error(`❌ Failed to migrate ${item.file}: ${error}`);
          report.errorFiles++;
          item.migrationStatus = 'error';
        }
      }
    }
  }

  private async migrateFile(item: LegacyContent): Promise<void> {
    const sourcePath = path.join(this.sourceDir, item.file);
    const targetPath = path.join(this.targetDir, item.targetLocation);
    ...
    console.log(`✅ Migrated: ${item.file} → ${item.targetLocation}`);
  }

  private createFrontmatter(item: LegacyContent): DocFrontmatter {
    return {
      title: item.title,
      description: item.description,
      domain: this.mapCategoryToDomain(item.category),
      type: this.determineContentType(item),
      layer: 'global',
      audience: this.determineAudience(item.category, item.priority),
      phase: 1,
      complexity: this.determineComplexity(item),
      freshness_review: this.calculateFreshnessDate(item.priority),
      validation_status: 'unverified',
      last_updated: new Date().toISOString().split('T')[0],
      tags: this.generateTags(item.category, item.title),
      legacy_path: item.file,
    };
  }
  ...
  private determineContentType(item: LegacyContent): 'tutorial' | 'how-to' | 'reference' | 'explanation' {
    // Based on content characteristics
    if (item.codeBlocks > 3) return 'how-to';
    if (item.category === 'architecture' || item.category === 'security') return 'reference';
    if (item.wordCount > 2000) return 'explanation';
    return 'how-to';
  }

  private determineAudience(category: string, priority: number): string[] {
    const audience = ['developer'];
    
    if (category === 'architecture') audience.push('architect');
    if (category === 'security') audience.push('devops');
    if (category === 'operations') audience.push('devops');
    if (priority >= 8) audience.push('qa');
    
    return audience;
  }

  private determineComplexity(item: LegacyContent): 'beginner' | 'intermediate' | 'advanced' {
    if (item.codeBlocks > 5 || item.wordCount > 2000) return 'advanced';
    if (item.codeBlocks > 0 || item.wordCount > 500) return 'intermediate';
    return 'beginner';
  }

  private calculateFreshnessDate(priority: number): string {
    const days = priority >= 8 ? 180 : priority >= 5 ? 270 : 365;
    const date = new Date();
  ...
  private async ensureDirectoryExists(dirPath: string): Promise<void> {
    try {
      ...
      await writeFile(dirPath, ''); // This will be replaced by directory creation
    }
  }
...
Content by Category:
${Object.entries(report.contentByCategory).map(([cat, items]) => `- ${cat}: ${items.length} files`).join('\n')}`;
  }
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '-h':
        console.log(`
Legacy Guides Migration Tool

Usage:
  tsx scripts/migrate-legacy-guides.ts [options]
...
Examples:
  tsx scripts/migrate-legacy-guides.ts
  tsx scripts/migrate-legacy-guides.ts --execute --priority-threshold 8
  tsx scripts/migrate-legacy-guides.ts --output-format markdown
        `);
        process.exit(0);
    }
  }
...
export { LegacyGuidesMigrator, type MigrationReport, type MigrationOptions };
