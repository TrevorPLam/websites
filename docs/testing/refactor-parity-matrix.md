...
| -------------------------------------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `clients/starter-template/__tests__/refactor-parity/*.test.ts` | Template-level integration | Critical paths using template wiring (e.g. `lib/search.ts`, booking page flow). Verifies template + @repo/features together. |
...
- **Command:** `pnpm test` (root Jest; includes all projects and `clients/starter-template/__tests__/refactor-parity/*.test.ts`).
...
- **Blocking:** Parity tests must pass before:
  - Removing template feature code (e.g. deleting `clients/starter-template/features/booking/lib/booking-actions.ts`).
  - Merging extraction PRs that change @repo/features contract.
- **Location:** `clients/starter-template/__tests__/refactor-parity/*.test.ts` is included in root `jest.config.js` (node project match patterns for template lib and feature tests).
...
2. Add or extend test file under `clients/starter-template/__tests__/refactor-parity/` or `packages/features/src/<feature>/__tests__/parity/`.
