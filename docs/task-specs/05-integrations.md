...
export interface EmailAdapter {
  subscribe(email: string, options?: { listId?: string; tags?: string[] }): Promise<{ ok: boolean; id?: string }>;
...
```
- All adapters implement EmailAdapter; retry/timeout in shared wrapper
...
const emailAdapterConfigSchema = z.discriminatedUnion('provider', [
  ...
  z.object({ provider: z.literal('convertkit'), apiKey: z.string(), formId: z.string().optional() }),
]);
```
