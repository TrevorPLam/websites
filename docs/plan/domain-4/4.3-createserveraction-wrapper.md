...
```typescript
// sites/[law-firm-client]/src/features/contact-form/model/submit.ts
'use server';
...
- **Task ID:** `DOMAIN-4-002`
- **Status:** ✅ Complete
- **Implementation references:** `packages/infra/security/secure-action.ts`, `packages/infra/__tests__/secure-action.test.ts`
- **Focused QA:** `pnpm vitest run packages/infra/__tests__/secure-action.test.ts`
