...
A domain-level type in `@repo/types/src/canonical/` that integrations translate to/from. Prevents integration-specific types from leaking into features. See [ADR-012](../adr/0012-canonical-types-data-contracts.md).

### Capability

A self-declaring feature unit (via `defineFeature`) that provides sections, integrations, and data contracts. Drives capability-based page composition in the universal renderer. Phase 3+. See [evolution-roadmap](../architecture/evolution-roadmap.md).
...
A compatibility layer that allows classic (registry-based) clients and the universal renderer to coexist. evol-10. See [evolution-roadmap](../architecture/evolution-roadmap.md).
...
An opt-in renderer (Phase 4) that composes pages from capabilities instead of the legacy section registry. Clients set `renderer: 'universal'` in site.config. evol-9. See [evolution-roadmap](../architecture/evolution-roadmap.md).
