...
2. **Build filter mismatch:** The Dockerfile used `pnpm run build --filter=@templates/hair-salon`, but the package name in `package.json` is `@templates/websites`. Turbo reports "No package found with name '@templates/hair-salon'", causing the build step to fail.
...
2. **Use path-based pnpm filter:** Change `--filter=@templates/hair-salon` to `--filter=./templates/hair-salon`. The path-based filter correctly targets the hair-salon template regardless of package name. Resolves the package name mismatch (see CODEBASE_AUDIT.md recommendation to rename to `@templates/hair-salon`).

3. **Preserve Dockerfile structure:** Next.js 15.5+ with pnpm workspaces produces standalone output at `.next/standalone/templates/hair-salon/` (directory-path nested). The existing COPY and CMD paths are correct:
   - `COPY --from=builder .../standalone ./` → places `templates/hair-salon/server.js` at `/app/templates/hair-salon/server.js`
   - `CMD ["node", "templates/hair-salon/server.js"]` → correct entry point
...
- `templates/hair-salon/Dockerfile`
