...
1. **Upgrade to Tailwind v4.1** across `clients/starter-template` and `packages/config`.
...
- `pnpm turbo run build --filter=./clients/starter-template` compiles and generates pages (standalone copy fails on Windows — pre-existing)
...
- clients/starter-template/app/globals.css
