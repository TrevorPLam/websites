// packages/seo/src/a11y-checklist.ts
...
export type WCAGChecklistItem = (typeof WCAG_22_AA_CHECKLIST)[number];

export function getAutomatedChecks() {
  return WCAG_22_AA_CHECKLIST.filter((item) => item.automated);
}
...
export function getWCAG22NewCriteria() {
  return WCAG_22_AA_CHECKLIST.filter((item) => item.criterion.includes('(WCAG 2.2 NEW)'));
}
