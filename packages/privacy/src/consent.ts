...
export async function acceptAllCookies(): Promise<void> {
  const cookieStore = await cookies();
  cookieStore.set(CONSENT_COOKIE, JSON.stringify(buildConsent({ analytics: true, marketing: true })), {
    httpOnly: false,
    sameSite: 'lax',
...
export async function rejectNonEssentialCookies(): Promise<void> {
  const cookieStore = await cookies();
  cookieStore.set(CONSENT_COOKIE, JSON.stringify(buildConsent({ analytics: false, marketing: false })), {
    httpOnly: false,
    sameSite: 'lax',
    secure: process.env.NODE_ENV === 'production',
    maxAge: CONSENT_MAX_AGE,
